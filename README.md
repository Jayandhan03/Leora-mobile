...
- **Not yet verified on a real device** — this dev environment has no
  ...
  below — do this before sharing the APK further.
- **The backend auth-bridge routes are written but need to be deployed.**
  `app/api/auth/mobile-bridge` and `app/api/auth/mobile-exchange` plus the
  `MobileAuthCode` model live in the main `D:\Leora` repo — push/deploy that
  repo to Vercel before testing sign-in, or the deep-link handoff has nothing
  to call.

## How Google Sign-In works here

Google blocks OAuth login inside embedded WebViews, so the sign-in button
hands off to a Chrome Custom Tab (a real browser) instead of navigating the
in-app WebView. That means **no new Google Cloud Console OAuth client is
needed** — it reuses the exact same Web OAuth client and redirect URI the web
app already has, since from Google's perspective this is a normal browser
completing a normal web login.

Flow:
1. App WebView → user taps "Continue with Google" → opens `/signin` in a
   Custom Tab (`lib/capacitor.ts` in the main repo).
2. User signs in with Google, entirely inside the Custom Tab — identical to
   the web flow.
3. NextAuth redirects to `/api/auth/mobile-bridge`, which mints a one-time
   code tied to the session that was just created, and redirects to
   `leora://auth-callback?code=...`.
4. Android hands that deep link back to the app (`AndroidManifest.xml`
   intent-filter + Capacitor's built-in `appUrlOpen` event —
   `components/CapacitorAuthBridge.tsx` in the main repo). Confirmed present
   in the final packaged manifest of the built APK.
5. The app's own WebView navigates to `/api/auth/mobile-exchange?code=...`,
   which redeems the code and sets the same session cookie inside the
   WebView. Same account, same session mechanism as web — nothing synced,
   nothing duplicated.

The one-time codes live in Mongo (`MobileAuthCode` model) with a 60-second TTL
index and are deleted on first use.
...
the repo root and re-run `npx cap sync android` before rebuilding.
...
3. From this machine:
   ...
   & "D:\Android\Sdk\platform-tools\adb.exe" install releases\leora-v1.0.apk
   ```
4. Open the "Leora" app on the phone, tap "Continue with Google", confirm it
   opens a Chrome tab (not an in-app popup), sign in, and confirm it lands
   back on `/dashboard` inside the app already signed in.
...
- Deploying the `mobile-bridge` / `mobile-exchange` routes to Vercel.
